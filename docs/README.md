# ftth-compete — Project documentation

This is the entry-point doc. **Keep it short.** Deeper context lives in the sibling docs in this folder.
...
| `/admin?key=<ADMIN_KEY>` | **Private visitor log.** SQLite-backed event store (`market_lookup`, `tab`, `lens`, `tract_click`, `provider_click`). Wrong / missing key → 404. |
...
- **Provider name canonicalization is a single source of truth** in `src/ftth_compete/data/providers.py`. Update both the code and `docs/providers.md` together.
...
- [admin-page-backlog.md](admin-page-backlog.md) — ideas + persistence options for expanding the `/admin` sidecar beyond the current event-log table
